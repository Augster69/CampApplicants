public class ApplicantProcessor {
    ...
    public Camper processApplicant(Applicant applicant){
       ...
       if(applicant.getAge() >=4 && applicant.getAge() <= 8){ 
        return new Camper(applicant.getFirstName(), applicant.getLastName(), applicant.getage());
       }
        if(applicant.getAge() > 8 && applicant.getAge() <= 13){
            return new Camper(applicant.getFirstName(), applicant.getLastName(), applicant.getage());
        }
        if(applicant.getAge() >13 && applicant.getAge() <=18){
            return new Camper(applicant.getFirstName(), applicant.getLastName(), applicant.getage());
        }
        return "Not applicable for camp. Sorry! ):";
    }
}
